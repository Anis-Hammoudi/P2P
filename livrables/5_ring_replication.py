...
def get_live_nodes_from_cluster():
    """Contact any random node to get its view of the cluster (SWIM-style)"""
...
if __name__ == "__main__":
    if len(sys.argv) == 2:
        ...
        # Run a Mini-Cassandra demo
        import subprocess
        processes = {}
        print("Démarrage du cluster Mini-Cassandra (5 nœuds)...")
        for port in ALL_PORTS:
        ...
        try:
            # We initialize the Coordinator from task3
            coordinator = Coordinator(get_live_nodes_func=get_live_nodes_from_cluster)
            
            key = "user_password"
            val = "secret123"
            
            print(f"\n[1] Client effectue un PUT sur '{key}' avec R=3...")
            coordinator.handle_client_put(key, val, R=3)
            
            print(f"\n[2] Client effectue un GET sur '{key}' avec R=3...")
            res = coordinator.handle_client_get(key, R=3)
            print(f" -> Résultat obtenu par le client : {res}")
